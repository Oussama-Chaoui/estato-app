...
import { Search, MapPin, Calendar, ChevronDown, ChevronUp, Home, Bed, Bath, Star, Wallet, ChevronsUpDown, Eraser, X } from "lucide-react";
...
interface DailyPropertiesFilterProps {
  onSearch: (params: {
    location?: string;
    checkIn?: Date;
    checkOut?: Date;
...
const DailyPropertiesFilter = forwardRef<any, DailyPropertiesFilterProps>(({
  onSearch,
  initialLocation,
  initialCheckIn,
  initialCheckOut,
  ...
  // Get max price limit based on website focus
  const getMaxPriceLimit = () => {
    switch (websiteFocus) {
  ...
  const activeFiltersCount = useMemo(() => {
    ...
    if (propertyType) count++; // propertyType is also applied immediately via auto-trigger
    ...
    return count;
  }, [location, dateRange, propertyType, appliedMinPrice, appliedIsMaxPriceActive, appliedMaxPrice, appliedBedrooms, appliedBathrooms, appliedAmenities]);
  ...
  }, [location, dateRange, propertyType]);
  ...
  const getActiveFilterLabels = useMemo(() => {
    ...
    if (dateRange?.from || dateRange?.to) {
      labels.push(t('filters.dates.placeholder'));
    }
    if (propertyType) {
    ...
    return labels;
  }, [location, dateRange, propertyType, minPrice, isMaxPriceActive, maxPrice, bedrooms, bathrooms, selectedAmenities, locationOptions, PROPERTY_TYPES, amenityOptions, t]);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <div className="flex items-center gap-2 px-3 py-2.5 border border-gray-200 rounded-xl bg-white hover:border-primary-500 transition-colors h-12">
              <DatePickerWithRange
                ...
                placeholder={t('filters.dates.placeholder')}
                className="flex-1 bg-transparent border-none shadow-none h-auto p-0 text-xs [&>button]:bg-transparent [&>button]:border-none [&>button]:shadow-none [&>button]:h-auto [&>button]:p-0 [&>button]:text-xs [&>button]:justify-start [&>button]:hover:bg-transparent [&>button]:focus:bg-transparent"
              />
            </div>
          </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="space-y-3">
                  <div className="flex items-center gap-2">
                    ...
                    <Label className="text-sm font-semibold text-gray-900">{t('filters.property_type.placeholder')}</Label>
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    {PROPERTY_TYPES.map((type) => (
                      <Button
                        ...
                        className={`justify-start h-10 text-xs ${propertyType === type.value
                ...
                <div className="space-y-3">
                  <Label className="text-sm font-semibold text-gray-900 flex items-center gap-2">
                    ...
                    {t('filters.price_range.title')} {websiteFocus === WEBSITE_FOCUS.DAILY_RENT ? t('filters.price_range.per_night') : websiteFocus === WEBSITE_FOCUS.RENT ? t('filters.price_range.per_month') : t('filters.price_range.total')}
                  </Label>
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    {/* Max Price Input */}
                    <div
                      className={`flex items-center gap-2 px-3 py-2.5 border rounded-xl transition-all h-12 cursor-pointer relative ${isMaxPriceActive
                      ...
                      onClick={() => {
                        if (!isMaxPriceActive) {
                          ...
                          const maxPriceLimit = getMaxPriceLimit();
                          const smartDefault = Math.min(minPrice + 500, maxPriceLimit);
                    ...
                    >
                      ...
                      {isMaxPriceActive ? (
                        <div className="flex-1 relative">
                          <Input
                            type="number"
                            placeholder={getMaxPriceLimit().toString()}
                            value={maxPrice || ''}
                            onChange={(e) => {
...
export default DailyPropertiesFilter; 
