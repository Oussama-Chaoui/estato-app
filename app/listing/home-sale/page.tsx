...
export const generateMetadata = async ({ searchParams }: { searchParams: { [key: string]: string | string[] | undefined } }): Promise<Metadata> => {
  ...
  const qs = extractAllowedParams(searchParams, [...PARAM_SETS.LISTING_SALE]);
  const page = parsePageNumber(qs);
  const filters = buildFiltersFromParams(qs, WEBSITE_FOCUS.SELLING);
  
  const result = await fetchListings(filters as ListingFilterInputs, page, 12, locale);
  
  const canonical = buildCanonical(AppRoutes.Properties.HomeSale.ReadAll, qs, [...PARAM_SETS.LISTING_SALE]);
  const hasActiveFilters = Boolean(filters.location || filters.propertyType || qs.get('priceMin') || qs.get('priceMax'));
  const listingMeta = buildListingMetadata({
    locale,
    focus: WEBSITE_FOCUS.SELLING,
    filters: { 
...
export default async function Page({ searchParams }: { searchParams: { [key: string]: string | string[] | undefined } }) {
  ...
  return (
    <>
      ...
      <PropertiesListing websiteFocus={WEBSITE_FOCUS.SELLING} initialData={initialData} />
    </>
  );
}
