...
export const generateMetadata = async ({ params }: Params): Promise<Metadata> => {
  ...
  const meta = buildPropertyMetadata(property, WEBSITE_FOCUS.DAILY_RENT, locale);

  const canonical = buildCanonical(AppRoutes.Properties.DailyRent.ReadOne.replace('{id}', params.id));
...
export default async function Page({ params }: Params) {
  ...
  return (
    <>
      ...
      <PropertyShowcase websiteFocus={WEBSITE_FOCUS.DAILY_RENT} initialProperty={property} />
    </>
  );
}
